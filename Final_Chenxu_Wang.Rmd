...
cardio_tree = cardio_clean_final[,c(1,2,3,4,13,5,6,7,8,9,10,11,12)]
str(cardio_tree)
...
for (deep in 2:8) {
  kfit <- rpart(cardio ~age+ap_lo+bmi+weight+height+cholesterol+gluc+gender+active+smoke+alco, data= cardio_tree, method="class", control = list(maxdepth = deep) )
  # 
...
As we can see from the results, the accuracy of different depths is almost same. Therefore, we chosen someone to build the tree model. 
...
set.seed(1)
cardio_tree_fit <- rpart(cardio ~ age+ap_lo+bmi+weight+height , data= cardio_tree, method="class", control = list(maxdepth = 6) )
...
library(rpart)
rp <- rpart(cardio ~age+ap_lo+bmi+weight+height, data = cardio_tree)
library(ROCR)
