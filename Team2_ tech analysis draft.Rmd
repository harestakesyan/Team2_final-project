...
### Statistical Tests  
We can use a two-sample t-test to determine if the differences in averages (for the continuous variables) are statistically significant.
...
corrplot(cardio_cor, type="lower", addCoef.col="darkred", number.cex=0.5, tl.cex=0.7,title="Correlation Matrix", mar=c(0,0,1,0))
```
...
hw <- ggplot(cardio_clean_final, aes(x=height, y=weight, color=cardio)) +
  scale_color_manual(values = c("Absent", "Present"))
ap2
