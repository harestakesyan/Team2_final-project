...
The p-value was 0 for all quantitative variables except for `height`, indicating that there is a statistically significant difference in the average `age`, `weight`, `bmi`, `ap_hi` and `ap_lo` of CVD and non-CVD patients. ***The p-value for height was very low as well, most likely because of the large sample size, because the rounded mean of both "CVD" and "no CVD" groups was 165cm.***

For all of the continuous variables, we reject the null hypothesis and accept that there is a difference between the means of the CVD and non-CVD groups.  The t-tests tests align with the observations from the density plots, as there was a noticeable difference in the means of all continuous variables except height.  
...
Odd numbers from 3 to 40 were tested one by one to get the best K value. The accuracy values from the model that used these K values are shown in the table and the line graph.
