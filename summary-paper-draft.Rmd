...
A summary of the cleaned dataset with basic statistics is presented below.  
...
## Chi-Squared Tests
We performed $\chi^2$ tests to further explore the data and determine if differences we observed between the "CVD" and "no CVD" groups were statistically significant.
...
The p-value for `gender` is greater than 0.05, so we can conclude that gender is independent of cardiovascular disease and may not have a significant impact on whether a patient has CVD or not. The p-values for all other variables are less than 0.01, in fact `cholesterol`, `glucose`, `smoke` and `active` are almost 0, so we reject the null hypothesis and conclude that these variables are related to cardiovascular disease. 

The $\chi^2$ tests agree with the observations from the box plots. The factors with the smallest p-values (cholesterol and glucose) are the same ones that had the most obvious differences in their plots.
...
Density plots are used to show the distribution of a continuous/quantitative variable in the dataset. Dashed lines were added to the plots to indicate the mean of the CVD (red) and non-CVD (blue) groups.  

We see from the plots that the distribution of patients with CVD is different from non-CVD patients for all variables except for `height`.  In general, patients with CVD are more likely to be older and weigh more than the "no CVD" cohort.  Both systolic and diastolic blood pressure tend to be higher in the "has CVD" group as well. 

## Welch Two-Sample T-Test
Two-sample t-tests are used to test whether the mean of the "CVD" and "no CVD" groups is different for each of the quantitative variables.
...
str(ttest_age)
format(ttest_age$estimate[1], digits=3)
...
The p-value for all continuous variables was 0, except for `height`, indicating that there is a statistically significant difference in the average `age`, `weight`, `bmi`, `ap_hi` and `ap_lo` of CVD and non-CVD patients. The p-value for height was very low as well, most probably due to the large sample size, because the rounded mean of both "CVD" and "no CVD" groups was 165cm.

For all of the continuous variables, we reject the null hypothesis and accept that there is a difference between the means of the CVD and non-CVD groups.  The t-tests tests agree with the observations from the density plots, as there was a noticeable difference in the means of all continuous variables except height.  
...
Some of these correlations are expected, for instance systolic blood pressure and diastolic blood pressure typically increase and decrease together. Height and gender are also correlated, which makes sense as men are typically taller than women.  While the correlation matrix does not show how the relationship between variables impacts CVD, it is important to keep these relationships in mind when we begin building models.   
...
chol1 <- ggplot(cardio_clean_final,aes(x = cholesterol , y = age, fill = cardio)) + 
  ...
  geom_boxplot(width=.1, cex=1, position=position_dodge(1))+
...
smoke1 <- ggplot(cardio_clean_final,aes(x = smoke , y = ap_hi, fill = cardio)) + 
  geom_violin(width=1, alpha=0.5, scale='width') + 
  geom_boxplot(width=.1, cex=1, position=position_dodge(1))+
...
alco1 <- ggplot(cardio_clean_final,aes(x = smoke , y = ap_hi, fill = cardio)) + 
  geom_violin(width=1, alpha=0.5, scale="width") + 
  geom_boxplot(width=.1, cex=1, position=position_dodge(1))+
  labs(title = "Systolic Blood Pressure vs Alcohol", fill="Has CVD") +
...
active1 <- ggplot(cardio_clean_final,aes(x = active , y = ap_hi, fill = cardio)) + 
  geom_violin(width=1, alpha=0.5, scale="count") + # Scale maximum width proportional to sample size
  geom_boxplot(width=.1, cex=1, position=position_dodge(.5))+
