...
#Gender
women.disease<- (sum(disease$gender==1)/nrow(disease)) * 100
women.healthy<- (sum(healthy$gender==1)/nrow(healthy)) * 100

women.disease - women.healthy

#Cholesterol
chol.disease<- (sum(disease$cholesterol!= "1")/nrow(disease)) * 100
chol.healthy<- (sum(healthy$cholesterol!= "1")/nrow(healthy)) * 100

chol.disease - chol.healthy

#Glucose
glu.disease<- (sum(disease$gluc!= "1")/nrow(disease)) * 100
glu.healthy<- (sum(healthy$gluc!= "1")/nrow(healthy)) * 100

glu.disease - glu.healthy

#Smoke
smo.disease<- (sum(disease$smoke==1)/nrow(disease)) * 100
smo.healthy<- (sum(healthy$smoke==1)/nrow(healthy)) * 100

smo.disease - smo.healthy

#Alcohol
alc.disease<- (sum(disease$alco==1)/nrow(disease)) * 100
alc.healthy<- (sum(healthy$alco==1)/nrow(healthy)) * 100

alc.disease - alc.healthy

#Active
act.disease<- (sum(disease$active==1)/nrow(disease)) * 100
act.healthy<- (sum(healthy$active==1)/nrow(healthy)) * 100

act.disease - act.healthy 
...
```  
Normal blood pressure for most adults is defined as a systolic pressure of less than 120 and a diastolic pressure of less than 80, elevated blood pressure is defined as a systolic pressure between 120 and 129 with a diastolic pressure of less than 80, high blood pressure is defined as 130 or higher for the first number, or 80 or higher for the second number. (NIH)[https://www.nia.nih.gov/health/high-blood-pressure-and-older-adults]. Because there is a strong correlation between systolic and diastolic blood pressure, we look at systolic in comparison to other variables: `cholesterol`, `age` and `bmi.`  
...
#### Modeling results with the best K value (K = 31)  

Detailed results and the confusion matrix using K=31 were shown below. The following information can be calculated from the confusion matrix:  
* Accuracy = 0.721  
* Precision = 0.737  
* Recall = 0.680  
* Specification = 0.761  
...
Compared to the models that with or without `gender` and `bmi`, the model only use `age`, `cholesterol`, `bmi`, `ap_hi` and `ap_lo`did not make too much improvement. The only advantage is the Recall value increased from 0.676 to 0.680. If we are looking for models that can better predict disease present, this model is more appropriate. But considering the tiny increase 0.4%, the difference might not very significant. **
