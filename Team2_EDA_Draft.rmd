...
```  
We see that cardiovascular disease has a positive correlation with blood pressure, cholesterol, age, and bmi.

### Will blood pressure influence cardiovascular disease with other variables?
```{r, results='markup'}
#library(patchwork)

ap1 <- ggplot(cardio_clean_final,aes(x = cholesterol , y = ap_hi , fill = cardio)) +
  geom_violin(alpha = 0.5, aes(linetype=NA)) +
  xlab("Cholesterol") + ylab("Systolic blood pressure") + 
  scale_fill_hue(labels = c("Absent", "Present"))
ap1

ap2 <- ggplot(cardio_clean_final, aes(x=age, y=ap_hi, color=cardio)) +
  geom_point(size=0.6,alpha=0.4) + 
  scale_colour_hue(labels = c("Absent", "Present"))
ap2
